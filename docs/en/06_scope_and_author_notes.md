...
In the current version of HSS, the main observation targets are connection structures that appear around L1–L6.
...
However, L1–L6 are not fixed layer definitions.
...
However, being able to describe or organize something does not mean that the model is objectively correct.

HSS is one observation coordinate system.

It is not a truth system that completely explains the world or humans.

Seeing a structure from one observation coordinate system and that structure being an objective truth are different matters.
...
HSS is not a personality classification model.

It is not a model for fixing a person as “this type.”

What HSS mainly observes are states, connections, routes, compression, and re-expansion.

For example, HSS handles questions such as:

- Which connection routes are becoming thicker?
- Which connection routes are becoming thinner?
- Which symbols are becoming fixed?
- Which routing is being captured by external indicators?
- Which layered history is being reconnected?
...
HSS is a structural memo for observing where stratified routing may be weakening, being compressed, being captured, or about to be cut off.
